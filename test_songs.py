"""Tests for the Songs feature (Gemini-powered original practice songs,
Max-only).

Focus: require_max rejects free/Pro users before any Gemini call is made,
the request sent to Gemini asks for structured JSON matching the theme/
level, and call_gemini_song() correctly parses a realistic response (plus
handles a malformed one without crashing). requests.post is faked
